...
const MessageDetail = ({ message, onClose, agentName }) => {
  const [reply, setReply] = useState('');
  const queryClient = useQueryClient();

  const replyMutation = useMutation({
    mutationFn: async ({ messageId, content, agentName }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (reply.trim()) {
      try {
        await replyMutation.mutateAsync({ messageId: message.id, content: reply, agentName });
      } catch (error) {
  ...
  const isMessageRepliedByOther = message.replies.some((reply) => reply.repliedBy !== agentName);
  ...
  return (
    <div className="h-screen flex flex-col">
        ...
        {message.replies?.map((reply) => (
          <div key={reply.id} className="bg-blue-50 p-4 rounded-lg shadow-sm mb-4 ml-8">
            <div className="flex justify-between items-start">
              ...
              <div className="text-xs text-gray-500">{reply.repliedBy}</div>
            </div>
      ...
      {isMessageRepliedByOther ? (
        <div className="p-4 border-t bg-gray-100 text-gray-500">
          This message has already been replied to by another agent. You cannot reply to it.
        </div>
      ) : (
